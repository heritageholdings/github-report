...
def get_pull_requests(repository_name: str) -> List[
    ...
    return list(map(lambda d: PullRequest(d), data))
...
def group_by_type(pull_requests: List[PullRequest]) -> Dict:
    """
    Heritage Pull Requests follow the conventional commit format, ex:
    feat: Add Button
    fix: Button is always disabled
    chore: add tests
    :param pull_requests:
    :return: a dictionary where the keys are the type and the value the list of related pull requestd
    """
    by_type = {}
    for pr in pull_requests:
        index_1 = pr.title.find(":")
        index_2 = pr.title.find("(")
        pr_type = "unknown"
        if index_1 >= 0 or index_2 >= 0:
            pr_type = pr.title[:min(filter(lambda i: i != -1, (index_1, index_2)))]
        by_type[pr_type] = by_type.get(pr_type, [])
        by_type[pr_type].append(pr)
    return by_type
